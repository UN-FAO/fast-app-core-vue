...
let ParallelSurvey = (() => {
  ...
  async function storeNewSurvey({ survey, vm }) {
    let formio = new Formio(
      vm.$FAST_CONFIG.APP_URL + "/" + vm.$route.params.idForm
    );
    // De register if there was a previous registration
    Formio.deregisterPlugin("offline");
    // Register the plugin for offline mode
    Formio.registerPlugin(
      OFFLINE_PLUGIN.getPlugin({ formio: formio, hashField: vm.hashField }),
      "offline"
    );
  ...
  function prepareNewGroupObject({ submission, vm, info }) {
    ...
    vm.currentSubmission.data.parallelSurvey = Submission.local().setParallelSurvey(
      parallelSurvey
    );
        ...
        inputValidator: value => {
          return new Promise((resolve, reject) => {
            if (value !== "") {
              ...
              let error = new Error(
                vm.$t("The group name is already taken")
              );
              reject(error);
            }
          });
  ...
  function getGroupId(submission) {
    let groupId = _get(
      Submission.local().getParallelSurvey(submission),
      "groupId",
      undefined
    );
    return groupId
