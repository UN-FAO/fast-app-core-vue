  ...
  <div class="tableContainer" v-if="show">
    ...
    >
      <template :slot="'col-' + col.field" scope="scope" v-for="col in columns">
        ...
        >{{scope.data ? scope.data : '-'}}</q-btn>
        <span v-bind:key="col.field" v-if="col.field !== 'HumanUpdated'">{{$t(scope.data)}}</span>
        <span v-bind:key="col.field" v-else>{{scope.data}}</span>
      </template>
